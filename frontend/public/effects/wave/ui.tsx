...
export default function WaveEffect(props: { deviceConfig?: DeviceConfig; onTick?: OnTickCallback }) {
  ...
  const [sat, setSat] = React.useState(92);
  const [val, setVal] = React.useState(83);
  ...
  const handleWaveWidthChange = (v: number) => {
    setWaveWidth(v);
    machine.setConfig('waveWidth', v);
  };
      ...
      <div className="space-y-4 border-t border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900/60">
        ...
        <div className="rounded-xl border border-slate-200 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-900/60">
          <div className="flex justify-between items-center mb-3">
            <label className="text-sm font-bold text-slate-600 dark:text-slate-400">Wave Width</label>
            <span className="rounded-md border border-slate-300 bg-white px-3 py-1 font-mono text-xs text-emerald-500 dark:border-slate-700 dark:bg-slate-900/60">
              {waveWidth}
            </span>
          </div>
          <FlutterCupertinoSlider
            min={1} max={10} value={waveWidth}
            onChange={(e: any) => handleWaveWidthChange(e.detail)}
            activeColor={accent}
          />
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-900/60">
          ...
          <FlutterCupertinoSlider
            min={50} max={500} step={50} value={speed}
