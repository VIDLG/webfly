...
export default function ChaseEffect() {
  // @ts-expect-error: WebF interop
  const { leds, speed, setSpeed, handleStart, handlePauseResume, handleStop, isIdle, isPaused } = useChaseEffect();
      ...
      <div className="flex-1 flex items-center justify-center bg-white/70 p-8 dark:bg-slate-900/60">
        <div className="flex flex-wrap justify-center gap-3 max-w-2xl">
          {leds.map((isOn: boolean, index: number) => (
            <div
              key={index}
              className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-slate-300 transition-colors duration-150 dark:border-slate-700 ${
                isOn
